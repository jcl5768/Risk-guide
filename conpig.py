# =============================================================================
# config.py — 섹터 설정 & 매핑 데이터
#
# ✅ 이 파일만 수정하면 됩니다:
#   - 새 섹터 추가          → SECTOR_CONFIG에 블록 추가
#   - 지표 가중치 조정      → "weight" 값 변경
#   - 새 종목 섹터 매핑 추가 → ETF_MAP에 티커 추가
#
# 다른 파일(engine.py, pages.py, main.py)은 건드릴 필요 없습니다.
# =============================================================================

# -----------------------------------------------------------------------------
# SECTOR_CONFIG
#   구조: SECTOR_CONFIG[섹터키] = {
#       "icon"       : 이모지,
#       "color"      : 대표 색상 (hex),
#       "label"      : 한국어 이름,
#       "cycle_note" : 이 섹터가 어떤 사이클에 연동되는지 한 줄 설명,
#       "indicators" : [
#           {
#             "name"   : 표시 이름,
#             "ticker" : yfinance 심볼,
#             "weight" : 승률 가중치 (양수=호재, 음수=악재, 절댓값=강도),
#             "desc"   : 왜 이 지표가 이 섹터에 중요한지 한 줄 설명
#           }, ...
#       ]
#   }
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# SECTOR_MAP — yfinance sector 문자열 → SECTOR_CONFIG 키
# -----------------------------------------------------------------------------
SECTOR_MAP = {
    "Energy":                 "Energy",
...
# -----------------------------------------------------------------------------
# ETF_MAP — 자주 쓰는 티커 → 섹터 직접 매핑 (yfinance API 호출 없이 즉시 처리)
# 새 종목 추가 시 여기에 한 줄만 추가하면 됩니다.
# -----------------------------------------------------------------------------
ETF_MAP = {
    # 에너지
    "XLE":"Energy",  "XOM":"Energy",  "CVX":"Energy",  "COP":"Energy",  "SLB":"Energy",  "EOG":"Energy",
    # 기술주
    "XLK":"Technology",  "AAPL":"Technology",  "MSFT":"Technology",
    "CRM":"Technology",  "ADBE":"Technology",  "ORCL":"Technology",  "ARKK":"Technology",
    # 반도체
    "NVDA":"Semiconductor",  "AMD":"Semiconductor",   "INTC":"Semiconductor",
    "QCOM":"Semiconductor",  "MU":"Semiconductor",    "SOXX":"Semiconductor",
    "SMH":"Semiconductor",   "AMAT":"Semiconductor",  "LRCX":"Semiconductor",
    "ASML":"Semiconductor",  "TSM":"Semiconductor",   "AVGO":"Semiconductor",  "ARM":"Semiconductor",
    # 경기소비재/EV
    "TSLA":"Consumer Cyclical",  "F":"Consumer Cyclical",    "GM":"Consumer Cyclical",
    "RIVN":"Consumer Cyclical",  "AMZN":"Consumer Cyclical", "HD":"Consumer Cyclical",
    "NKE":"Consumer Cyclical",   "XLY":"Consumer Cyclical",  "LIT":"Consumer Cyclical",
    # 필수소비재
    ...
    "MCD":"Consumer Defensive",  "PM":"Consumer Defensive",
    # 금융
    ...
    # 산업재
    "XLI":"Industrials",  "CAT":"Industrials",  "DE":"Industrials",
    "BA":"Industrials",   "GE":"Industrials",   "UPS":"Industrials",  "RTX":"Industrials",
    # 부동산
    "XLRE":"Real Estate",  "AMT":"Real Estate",  "PLD":"Real Estate",  "EQIX":"Real Estate",
    # 유틸리티
    "XLU":"Utilities",  "NEE":"Utilities",  "DUK":"Utilities",  "SO":"Utilities",
    # 소재
    "XLB":"Basic Materials",  "FCX":"Basic Materials",
    "NEM":"Basic Materials",  "LIN":"Basic Materials",  "GLD":"Basic Materials",
    # 통신/미디어
...
# -----------------------------------------------------------------------------
# 주요 거시 지표 — 메인 대시보드 상단 5개
# (label, yfinance ticker, 짧은 설명)
# -----------------------------------------------------------------------------
MACRO_INDICATORS = [
    ("S&P 500",     "^GSPC",     "시장 전체"),
    ("나스닥 100",  "^NDX",      "기술·성장주"),
...
# -----------------------------------------------------------------------------
# Lv.3 개발 로드맵 — pages.py의 심화 탭에서 사용
# -----------------------------------------------------------------------------
LV3_ROADMAP = [
    ("🔗", "상관계수 분석",         "종목 간 상관관계 정량화. XLE↔SPY, TSLA↔QQQ 등 분산 효과 측정",              "다음 개발"),
    ("🎲", "몬테카를로 시뮬레이션", "180일 1,000회 가상 실행 → 수익 분포도, VaR(최대 손실 추정)",                 "개발 예정"),
    ("🧮", "베이지안 업데이트",     "이전 확률 + 새 지표 → 수학적 확률 갱신 (단순 합산 아님)",                    "개발 예정"),
    ("📉", "변동성(σ) 계산",        "최근 시장 변동성 수치화 → 리스크 범위 자동 설정",                            "개발 예정"),
    ("🤖", "ML 회귀 모델",          "모든 특징값 학습 → 내일 기대 수익률 예측 + 백테스팅",                        "장기 계획"),
]
            {"name": "10년물 금리",    "ticker": "^TNX",     "weight": -10, "desc": "CAPEX 비용 — 파운드리 설비 확장에 영향"},
            {"name": "달러 인덱스",    "ticker": "DX-Y.NYB", "weight": -6,  "desc": "TSMC 등 아시아 공급망 원가·환율"},
            {"name": "구리 선물",      "ticker": "HG=F",     "weight": +5,  "desc": "전자부품 원자재 + 경기 선행지표"},
